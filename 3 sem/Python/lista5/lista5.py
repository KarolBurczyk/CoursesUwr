...
class Sub(Exp):
    ...
    def derivative(self):
        if len(set(self.exp1.var_count() + self.exp2.var_count())) <= 1:
            return Div(Sub(Mult(self.exp1.derivative(), self.exp2), self.exp1, self.exp2.derivative()), Mult(self.exp2, self.exp2))
        else:
            raise DerivativeError("Too many variables in the expression")
    def var_count(self):
        return self.exp1.var_count() + self.exp2.var_count()
...
class Mult(Exp):
    ...
    def derivative(self):
        if len(set(self.exp1.var_count() + self.exp2.var_count())) <= 1:
            return Add(Mult(self.exp1.derivative(), self.exp2), Mult(self.exp1, self.exp2.derivative()))
        else:
            raise DerivativeError("Too many variables in the expression")
    def var_count(self):
        return self.exp1.var_count() + self.exp2.var_count()
