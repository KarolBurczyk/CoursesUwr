...
(define (bindM [c : (M 'a)] [f : ('a -> (M 'b))]) : (M 'b)
  (cond [(empty? c) '()]
        [else (append (f (first c)) (bindM (rest c) f))]))
...
(define (showM [c : (M Value)]) : String
  (cond [(empty? c) ""]
        [else (string-append (value->string (first c)) (showM (rest c)))]))
...
(define (value->string [v : Value]) : String
  (type-case Value v
    [(numV n) (string-append (to-string n) " ; ")]
    [(funV _) "#<procedure>"]))
...
(define (eval [e : Exp] [env : Env])
  (type-case Exp e
    ...
    [(opE o e1 e2)
     (bindM (eval e1 env)
            (lambda (v1) (bindM (eval e2 env)
                           (lambda (v2) ((op->proc o) v1 v2)))))]
    [(varE x)
     (lookup-env x env)]
    [(lamE x b)
     (returnM (funV (lambda (v) (eval b (extend-env env x v)))))]
    [(appE e0 e1)
     (bindM (eval e0 env)
            (lambda (v0) (bindM (eval e1 env)
                           (lambda (v1) (apply v0 v1)))))]
    [(ambE e0 e1)
     (returnM (numV 0))]))
