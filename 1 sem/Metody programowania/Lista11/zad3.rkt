...
(define-type Exp
  ...
  (varE [x : Symbol])
  (letE [l : (Listof(Symbol * Exp))] [e : Exp])
  (let*E [l : (Listof(Symbol * Exp))] [e : Exp]))
    ...
    [(cons s ss)
     (if (s-exp-match? `{SYMBOL ANY} s)
         (cons (pair (s-exp->symbol (first (s-exp->list s)))
                     (parse (second (s-exp->list s))))
               (parse-let ss))
         (error 'parse-exp "invalid argument"))]))
...
(module+ test
  ...
  (test (parse `{let* {{x 1} {y (+ x 2)} {z (* y 3)}} {+ x y}})
        (let*E (list (values 'x (numE 1)) (values 'y (opE (add) (varE 'x) (numE 2))) (values 'z (opE (mul) (varE 'y) (numE 3))))
...
(define (update [l : (Listof (Symbol * Exp))] [env : Env])
...
(define (update* [l : (Listof (Symbol * Exp))] [env : Env])
    (cond [(empty? l) env]
          [else (update* (rest l) (extend-env env
                            (fst (first l))
                            (eval (snd (first l)) env)))])
  )
...
(module+ test
  ...
  (test (run `{let {{x 1}}
                {+ x {let {{x {+ x 1}}}
                       {* x 3}}}})
        (numV 7))
  (test (run `{let* {{x 1} {y (+ x 2)} {z (* y 3)}} {+ x y}})
        (numV 4)))
