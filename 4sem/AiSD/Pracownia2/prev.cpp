...
Mini brute(const pair<int, int> x[], int begin, int end) {
    Mini mini{};
...
int main() {
    ...
    Mini rek(const pair<int, int> x[], const pair<int, int> y[], int begin, int end, int n) {
        ...
        for (long unsigned int i = 0; i < strip.size() - 2; i++) {
            for (long unsigned int j = i + 1; j < strip.size() - 1 && abs(y[strip[j]].second - y[strip[i]].second) < d; j++) {
                for (long unsigned int k = j + 1; k < strip.size() && abs(y[strip[k]].second - y[strip[i]].second) < d; k++) {
