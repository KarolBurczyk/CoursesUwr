...
(define (fv-env [n : Symbol] [env : Env])
  (type-case (Listof Binding) env
    [empty #t]
    [(cons b rst-env) (cond
                        [(eq? n (bind-name b))
                         #f]
                        [else (fv-env n rst-env)])]))
...
(define (run-fv [e : S-Exp])
  (fv (parse e) mt-env))

(define (fv [e : Exp] [env : Env])
  (type-case Exp e
    [(numE n) '()]
    [(opE o l r) (append (fv l env) (fv r env))]
    [(ifE b l r)
     (append (append (fv b env) (fv l env)) (fv r env))]
    [(varE x)
     (if (fv-env x env) (list x) '())]
    [(letE x e1 e2)
