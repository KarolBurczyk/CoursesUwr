...
(define (decompile-pom nums lst)
  (cond [(empty? lst) (first nums)]
        [else (type-case Instr (first lst)
                [(pushI n) (decompile-pom (cons (numE n) nums) (rest lst))]
                [(opI op) (decompile-pom (cons (opE op (first (rest nums)) (first nums)) (rest (rest nums))) (rest lst))])
              ])
  )
